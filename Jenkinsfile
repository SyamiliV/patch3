...
stage ('build') {
    ...
    sshPublisher(publishers: [sshPublisherDesc(configName: 'dockerhost', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: 'cd /home/dockerdeploy/webapp/target;docker stop "dockercontainers";docker rm "dockercontainers";docker rmi -f dockerimage;docker build -t dockerimage .;docker run -it -d --name "dockercontainers" -p 8080:8080 dockerimage', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '.', remoteDirectorySDF: false, removePrefix: '', sourceFiles: 'webapp/target/*.war')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])
